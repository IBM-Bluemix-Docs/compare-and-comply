...
```bash
curl -X DELETE -u "apikey":"{apikey_value}" https://{ICP_IP_address}:{port}/{deployment-name}/compare-and-comply/api/v1/feedback?version=2018-10-15
```
{: pre}
...
```bash
curl -X DELETE -u "apikey":"{apikey_value}" -H 'x-watson-metadata: customer_id=3910' https://{ICP_IP_address}:{port}/{deployment-name}/compare-and-comply/api/v1/feedback?version=2018-10-15
```
{: pre}
