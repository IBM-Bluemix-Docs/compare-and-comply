...
Antes de instalar el diagrama Helm de {{site.data.keyword.cnc_long}} tal como se describe en el archivo `README.md` del diagrama Helm, es posible que tenga que realizar pasos de despliegue adicionales. Debe realizar estos pasos únicamente si está desplegando la versión de {{site.data.keyword.cnc_long}} 1.0.3 o anterior. Estos pasos no se aplican si está desplegando la versión de {{site.data.keyword.cnc_long}} 1.0.4 o posterior.
...
Prepare un shell `bash` o equivalente para utilizar con la instalación de IBM Cloud Private ejecutando los mandatos siguientes en el shell:
  ...
  **Nota**: En el primer mandato de la lista anterior, el número de puerto `8443` en el URL `https://mycluster.icp:8443` representa el valor predeterminado para comunicarse con el servicio de {{site.data.keyword.cnc_short}} en IBM Cloud Private.

### Paso 5: Cree un secreto de registro de imágenes y una cuenta de servicio de parches
{: #step5}
...
donde:
  ...
  - `$secret_name` es una serie que especifica [para permitir a los contenedores de Docker comunicarse de forma segura ![Icono de enlace externo](../../icons/launch-glyph.svg "Icono de enlace externo")](https://docs.docker.com/engine/swarm/secrets/){: new_window}.

### Paso 6: Cargue el archivo de paquete en el registro de imágenes interno de IBM Cloud Private
{: #step6}

En el terminal que ha preparado en el [Paso 4](#step4), ejecute el siguiente mandato para cargar el archivo de imagen empaquetada:
