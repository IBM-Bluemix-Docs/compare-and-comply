...
Bevor Sie das Helmdiagramm für {{site.data.keyword.cnc_long}} wie in der Datei `README.md` des Helmdiagramms beschrieben installieren, müssen Sie möglicherweise noch einige zusätzliche Schritte für die Bereitstellung ausführen. Sie müssen diese Schritte nur ausführen, wenn Sie {{site.data.keyword.cnc_long}} Version 1.0.3 oder früher implementieren. Diese Schritte gelten nicht, wenn Sie {{site.data.keyword.cnc_long}} Version 1.0.4 oder höher implementieren.
...
### Schritt 3: Paketdatei herunterladen
{: #step3}
...
**Hinweis**: Wenn Sie die Prozedur in diesem Schritt ausführen, können Probleme mit Upgrades und Rollbacks von {{site.data.keyword.cnc_short}} Version 1.0.3 auftreten. 
  ...
  **Aktualisierter Inhalt**
  ...
  Zum Vergleich sieht die Ansicht `tree` (Baumstrukturansicht) des Inhalts der dekomprimierten Datei `IBM_WTSN_COMPARE_AN_COMPL_ELEM_CL.tar.gz` wie folgt aus. Dateien, die an dieser Prozedur beteiligt sind, werden mit `<-----` in der rechten Spalte aufgerufen.
...
Dabei gilt Folgendes:
  - `$username` und `$password` variieren je nach Cluster. Wenden Sie sich diesbezüglich an Ihren Administrator.
  - `$secret_name` ist eine von Ihnen angegebene Zeichenfolge [zum Aktivieren von Docker-Containern für die sichere Kommunikation ![Symbol für externen Link](../../icons/launch-glyph.svg "Symbol für externen Link")](https://docs.docker.com/engine/swarm/secrets/){: new_window}.

### Schritt 6: Paketdatei in die interne IBM Cloud Private-Image-Registry hochladen
{: #step6}

Führen Sie in dem in [Schritt 4](#step4) vorbereiteten Terminal den folgenden Befehl aus, um die gepackte Imagedatei hochzuladen:
...
Kehren Sie in Ihrem IBM Cloud Private-Cluster zur Seite **Katalog** zurück, suchen Sie den Eintrag `ibm-watson-compare-comply-prod` und wählen Sie ihn aus und klicken Sie anschließend auf **Konfigurieren**, um mit der Installation fortzufahren.
