...
Helm chart の `README.md` ファイルの説明に従って {{site.data.keyword.cnc_long}} Helm chart をインストールする前に、追加のデプロイメント・ステップを実行しなければならない場合があります。これらのステップは、{{site.data.keyword.cnc_long}} バージョン 1.0.3 以前をデプロイする場合にのみ実行する必要があります。{{site.data.keyword.cnc_long}} バージョン 1.0.4 以降をデプロイする場合、これらのステップは適用されません。
...
{: #step1}
  ...
  1. IBM Cloud Private レジストリーへのアクセス権限を Docker に付与します。
    ...
    - Linux で実行する場合は、以下のステップを実行します。
      ...
      1. `daemon.json` ファイルを見つけます。 デフォルトでは、このファイルは `/etc/docker/daemon.json` にあります。 ファイルが存在しない場合は、デフォルトの場所に作成してください。 テキスト・エディターでファイルを開き、次の JSON を確認するか追加します。
...
### ステップ 2: IBM Cloud Private CLI と IBM Cloud CLI のダウンロード
{: #step2}
...
**注**: このステップの手順を実行すると、{{site.data.keyword.cnc_short}} バージョン 1.0.3 のアップグレードとロールバックで問題が発生する可能性があります。 
  ...
  **更新後**
  ...
  参考までに、圧縮解除された「IBM_WTSN_COMPARE_AN_COMPL_ELEM_CL.tar.gz」ファイルの内容の「tree」ビューは、次のようになります。この手順に関連するファイルは、右側の列の「<-----」で示しています。
...
「bash」シェルまたはそれと同等のものを IBM Cloud Private インストール済み環境で使用できるように準備するために、シェルで次のコマンドを実行します。
  ```bash
...
各部の意味は、次のとおりです。
  -「$username」と「$password」は、クラスターによって異なります。管理者にお問い合わせください。
...
[ステップ 4](#step4) で準備した端末で次のコマンドを実行して、パッケージ化されたイメージ・ファイルをアップロードします。
...
IBM Cloud Private クラスターの「カタログ」ページに戻り、「ibm-watson-compare-comply-prod」という項目を見つけて選択し、「構成」をクリックしてインストールを進めます。
